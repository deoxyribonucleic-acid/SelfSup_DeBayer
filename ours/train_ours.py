...
parser.add_argument("--noisetype", type=str, default="gauss25", choices=['gauss25', 'gauss5_50', 'poisson30', 'poisson5_50', 'bilinear', 'hq'])
...
parser.add_argument('--pvr', action='store_true')
...
bayer_interp_method = False
if opt.noisetype in ['bilinear', 'hq']:
    bayer_interp_method = True
...
class AugmentNoise(object):
    ...
    def add_train_noise(self, x):
        ...
        if self.style == "gauss_fix":
            ...
            bayer = pp.bayer_filter_tensor(x)
            interp = pp.interp_tensor(bayer)
            interp.to(x.device)
            return interp
        elif self.style == "hq":
            bayer = pp.bayer_filter_tensor(x)
            interp = pp.hq_interp(bayer)
            interp.to(x.device)
            return interp
    ...
    def add_valid_noise(self, x):
        ...
        if self.style == "gauss_fix":
            ...
            bayer = pp.bayer_filter_ndarray(x)
            interp = pp.interp_ndarray(bayer)
            return interp
        elif self.style == 'hq':
            bayer = pp.bayer_filter_ndarray(x)
            interp = pp.hq_interp(bayer, tensor=False)
            return interp
...
for epoch in range(epoch_init, opt.n_epoch + 1):
    ...
    for iteration, clean in enumerate(TrainingLoader):
        ...
        net_input, mask = masker.train(noisy)  # (64, 3, 128, 128)
        if bayer_interp_method and opt.pvr:
            bayer = pp.bayer_filter_tensor(noisy) # (4, 3, 128, 128)
            bayer = bayer.repeat_interleave(16, dim=0)  # (64, 3, 128, 128)
            bayer = bayer.to(device)
            net_input[bayer != 0] = bayer[bayer != 0]
        ...
        loss_reg = alpha * torch.mean(diff**2) * 100
        loss_rev = torch.mean(revisible**2) * 100
    ...
    if epoch % opt.n_snapshot == 0 or epoch == opt.n_epoch:
        ...
        for valid_name, valid_images in valid_dict.items():
                ...
                for idx, im in enumerate(valid_images):
                    ...
                    if epoch == opt.n_snapshot:
                        ...
                        noisy255 = np.clip(noisy255 * 255.0 + 0.5, 0,
                                           255).astype(np.uint8)
                        if bayer_interp_method:
                            psnr_interp = calculate_psnr(origin255.astype(np.float32),
                                                         noisy255.astype(np.float32))
                            avg_psnr_interp.append(psnr_interp)
                            ssim_interp = calculate_ssim(origin255.astype(np.float32),
                                                         noisy255.astype(np.float32))
                            avg_ssim_interp.append(ssim_interp)
                    ...
                    with torch.no_grad():
                        ...
                        net_input, mask = masker.train(noisy_im) # (16, 3, H, W)
                        if bayer_interp_method:
                            bayer = pp.bayer_filter_tensor(noisy_im) # (1, 3, H, W)
                            bayer = bayer.to(device)
                            if opt.pvr:
                                bayer_expand = bayer.detach().clone().repeat_interleave(16, dim=0) #  (16, 3, H, W)
                                net_input[bayer_expand != 0] = bayer_expand[bayer_expand != 0]
                        noisy_output = (network(net_input) *
                                        mask).view(n, -1, c, h, w).sum(dim=1)
                        dn_output = noisy_output.detach().clone() #  (1, 3, H, W)
                        if opt.noisetype == 'hq': # I didn't use this when testing bilinear method!
                            dn_output[bayer != 0] = bayer[bayer != 0]
                        # Release gpu memory
                        del net_input, mask, noisy_output
                        torch.cuda.empty_cache()
                        exp_output = network(noisy_im) # (1, 3, H, W)
                        if opt.noisetype == 'hq':  # I didn't use this when testing bilinear method!
                            exp_output[bayer != 0] = bayer[bayer != 0]
            ...
            if epoch == opt.n_snapshot and bayer_interp_method:
                avg_psnr_interp = np.array(avg_psnr_interp)
                avg_psnr_interp = np.mean(avg_psnr_interp)
                avg_ssim_interp = np.mean(avg_ssim_interp)
                with open(log_path, "a") as f:
                    f.writelines(",,,interp:{:.6f}/{:.6f}\n".format(avg_psnr_interp, avg_ssim_interp))
