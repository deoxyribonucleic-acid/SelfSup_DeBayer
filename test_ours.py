...
for valid_name, valid_images in valid_dict.items():
        ...
        for idx, im in enumerate(valid_images):
            ...
            with torch.no_grad():
                ...
                if opt.noisetype == 'hq': # I didn't use this when testing bilinear method!
                    noisy_output[bayer != 0] = bayer[bayer != 0]
            ...
            logger.info(
                "{} - img:{}_{:03d} - PSNR_INTERP: {:.6f} dB; SSIM_INTERP: {:.6f}; PSNR_DN: {:.6f} dB; SSIM_DN: {:.6f}; PSNR_EXP: {:.6f} dB; SSIM_EXP: {:.6f}; PSNR_MID: {:.6f} dB; SSIM_MID: {:.6f}.".format(
                valid_name, i, idx, psnr_interp, ssim_interp, psnr_dn, ssim_dn, psnr_exp, ssim_exp, psnr_mid, ssim_mid
                )
            )
    ...
    if bayer_interp_method:
        ...
        logger.info(
            "----Average PSNR/SSIM results for {}----\n----PSNR_INTERP: {:.6f} dB; SSIM_INTERP: {:.6f}\n----PSNR_DN: {:.6f} dB; SSIM_DN: {:.6f}\n----PSNR_EXP: {:.6f} dB; SSIM_EXP: {:.6f}\n----PSNR_MID: {:.6f} dB; SSIM_MID: {:.6f}".format(
                valid_name, avg_psnr_interp, avg_ssim_interp, avg_psnr_dn, avg_ssim_dn, avg_psnr_exp, avg_ssim_exp, avg_psnr_mid, avg_ssim_mid
            )
        )
    else:
        logger.info(
